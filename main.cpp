...
int main() {
    fstream grib;
    // Otwarcie pliku w trybie do odczytu oraz trybie binarnym
    ...
    std::string nameCentre = (Centre == 7) ? "US National Weather Service - NCEP(WMC)" : "404 not found";
    ...
    std::string nameidprocess = (ProcessID == 81) ? "Analysis from GFS (Global Forecast System)" : "404 not found";
    ...
    string GDS = (GDSandBMS & 0b00000001) ? "GDS Included" : "GDS Omitted";
    string BMS = (GDSandBMS & 0b00000010) ? "BMS Included" : "BMS Omitted";
    ...
    int Lo1_millidegrees = (static_cast<int>(static_cast<unsigned char>(byte_chars4[0])) << 16) | (static_cast<int>(static_cast<unsigned char>(byte_chars4[1])) << 8) | static_cast<int>(static_cast<unsigned char>(byte_chars4[2]));
    ...
    outputBuffer << (isWestLongitude ? "-" : "") << Lo1_degrees << " degrees\n";
